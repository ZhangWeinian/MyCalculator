...
_NORETURN void myCalc::onButtonGroupCliked(const QAbstractButton* Btn)
{
	...
	// 处理事件
	inspectionStr_InputTime(defPair(text, type));
}
...
// 1.3 部分键盘文本转换
_NODISCARD qstr myCalc::getKeyboardText(const QKeyEvent* KeyEvent)
{
	qstr ans = "";
...
_NORETURN void myCalc::inspectionStr_InputTime(const ClickEvent& ThisTimeEvent)
{
	...
	// case2：检查输入的数字是否过大
	if (tmp && cove_type(getFloat(getDisplayingStr()), LL) > _NUM_MAX)
	{
...
// 2.3 处理数字
void myCalc::handleNum(const qstr& NumEvent)
{
...
// 2.4 处理基础操作符
void myCalc::handleBasicOper(const qstr& BasicOperEvent)
{
...
// 2.5 处理高级操作符
_NORETURN void myCalc::handleAdvancedOper(const qstr& AdvancedOperEvent)
{
	...
	if (cove_type(getFloat(info->Num), LL) > _NUM_MAX)
	{
		info->Num = "计算结果超出最大值";
...
// 3.1 记录数据
void myCalc::takeData(qstr&& Data, const BtnType& _Type, bool ClearData)
{
	Data = _STD move(Data);
...
// 4.2 获取一个高级操作符表达式
_NODISCARD qstr myCalc::getAdvancedStr(const qstr& Event, const qstr& keyNum) const
{
	qstr showStr = "";
...
// 4.3 获取整个表达式的计算结果
_NODISCARD qstr myCalc::calcStr(void)
{
...
// 4.4 获取指定表达式的计算结果
_NODISCARD qstr myCalc::calcStr(_VEC_QSTR& NumsVec, _SK_QSTR& OperSk)
{
	try
	{
		...
		if (cove_type(getFloat(OperSk.top()), LL) > _NUM_MAX)
		{
...
void myCalc::calcBracketStr(void)
{
	...
	auto strB = cove_type(info->HouZhui.size() - strE, int);

	_VEC_QSTR Vec = info->HouZhui.mid(strB, strE);
...
_NORETURN void myCalc::showInfo(const ClickEvent& Event)
{
	switch (Event.second)
	{
		case BtnType::_Num:
		{
			if (ui.line2->text() == "0" && Event.first != ".")
			{
				ui.line2->clear();
					 ...
					 info->lastTimeEvent->second != BtnType::_Num)
			{
				ui.line2->clear();
			}
		...
		{
			ui.line1->setText(info->ZhongZhuiStr + "  ");
			ui.line2->setText(info->HouZhui.back());
...
void myCalc::showAdvancedOperInfo(const ClickEvent& Event)
{
	...
	qstr showStr = getAdvancedStr(Event.first, keyNum);
